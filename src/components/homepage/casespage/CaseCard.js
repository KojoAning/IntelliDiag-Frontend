...
function CaseCard({ patient }) {
  ...
  const { name, age, gender, urgency, reason, mrn, appointmentTime, created_at,updated_at } = patient;

  return (
    <motion.div
      onClick={() => navigate(`/cases/${patient.id}`, { state: { patient } })}
      className="group bg-[#161616] rounded-2xl px-6 py-4 flex items-center gap-5 border border-[#1E1E1E] hover:border-[#0694FB] cursor-pointer outline-none"
    ...
    >
      ...
      <div className="w-[160px] shrink-0">
        ...
        <p className="text-[#6B6B6B] text-[13px] m-0 mt-0.5">{gender}</p>
      </div>
      ...
      <div className="hidden md:block flex-1 min-w-0">
        <p className="text-[#6B6B6B] text-[13px] m-0">Reason</p>
        <p className="text-[#e7e7e7] text-[14px] m-0 mt-0.5 truncate">{reason}</p>
      </div>

      {/* MRN — hidden on small+md, visible lg+ */}
      <div className="hidden md:block w-[100px] shrink-0">
        <p className="text-[#6B6B6B] text-[13px] m-0">MRN #</p>
        <p className="text-[#e7e7e7] text-[14px] m-0 mt-0.5">{mrn}</p>
      </div>
      ...
      {/* Date created */}
      <div className="hidden md:block w-[140px] pl-2 shrink-0">
        <p className="text-[#6B6B6B] text-[13px] m-0">Date Created</p>
        <p className="text-[#e7e7e7] text-[14px] m-0 mt-0.5">{fmtDate(created_at)}</p>
      </div>
      <div className="hidden md:block w-[140px] pl-2 shrink-0">
        <p className="text-[#6B6B6B] text-[13px] m-0">Last Updated</p>
        <p className="text-[#e7e7e7] text-[14px] m-0 mt-0.5">{fmtDate(updated_at)}</p>
      </div>
